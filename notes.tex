...
In this section, we fix a finite group $G$ with an abelian subgroup $K$ of odd order $\frac{|G|}{2}$ and let $H=|x|\in\Syl_2(G)$ with $|x|=2$ (as $|G|=2\times$odd number).
...
\end{lemma}
\begin{proof}
For a sanity check, we need to see $[K,x]$ is abelian for $\phi$ to make sense.
\begin{enumerate}
\item It suffices to show it for $k=[v,x]$, a generator of $[K,x]$.
\item 
\item 
\end{enumerate}
\end{proof}
